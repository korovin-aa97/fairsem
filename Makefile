...
test-containers:
	@for image in python:3.10-slim python:3.11-slim python:3.12-slim python:3.13-slim; do \
		echo "testing $$image"; \
		docker run --rm -v "$$PWD:/work" -w /work "$$image" python3 -m unittest -v tests.test_fairsem || exit 1; \
	done

lint:
	python3 -m py_compile bin/fairsem tests/test_fairsem.py tests/helpers/*.py scripts/build_release.py
